...
sys.path.append("/home/GEOTHERMIE/.local/lib/python3.5/site-packages")
...
class DataHandle():
    ...
    def getVerbrauch(self):
        ret = sum([entry[0] for key, entry in self.__verbraucher.items() if (entry[2] == 1 and key != "Fernseher") or (entry[2] == 0 and key == "Fernseher")])
        return str(ret) 
    ...
    def run(self):
        ...
        while not dHandle.getCloseServer():
            ...
            if "{" in str(copyOfState):
                ...
                for q in range(8):
                    for i in range(0, self.strip.numPixels(), 8):
                        self.strip.setPixelColor(i+q, Color(0, 255, 0))
                        self.strip.setPixelColor(i+q+1, Color(0, 255, 0))
                        self.strip.setPixelColor(i+q+2, Color(0, 255, 0))
                        self.strip.setPixelColor(i+q+3, Color(0, 255, 0))
                    self.strip.show()
...
class UserData():
    def __init__(self):
        ...
        with open("/home/GEOTHERMIE/ies/input.txt", "r") as getInput:
...
def handleData():
    ...
    if dHandle.input[0].strip() == "turnoff":
        if dHandle.getState("Fernseher") == 0:
            dHandle.invertState("Fernseher")
...
while not dHandle.getCloseServer():
    ...
    with open("/var/www/html/input/request.txt", "r") as file:
        for line in islice(file, textCount, sum(1 for line in open("/var/www/html/input/request.txt"))):
            ...
            if dHandle.output[5] == "none" and line.strip() != "" and not dHandle.getCloseServer():
                try:
                    if (dHandle.getState(line.strip().split(" ")[1]) == 0 and line.strip().split(" ")[1] == "Lampe") or (dHandle.getState(line.strip().split(" ")[1]) == 1 and line.strip().split(" ")[1] == "Fernseher"):
    ...
    dHandle.output[0] = str(int(((0.15*int(dHandle.output[4]))+(0.85*int(dHandle.output[3])))/200*((((int(dHandle.output[1])-int(dHandle.output[2]))**2)/100)**0.5)*((int(dHandle.output[1])-int(dHandle.output[2]))/12577280)))
